...
from diffusers import AutoencoderKL, DDPMScheduler, DiffusionPipeline, UNet2DConditionModel, StableDiffusionPipeline, PNDMScheduler
...
def wandb_cfg(args):
    # SETUP WANDB
    config_dict = {
        "dataset_name": args.instance_data_dir,
        "seed": args.seed,
...
def import_model_class_from_model_name_or_path(pretrained_model: str, revision: str):
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--save_images_steps", type=int, default=50, help="How often to save images during training."
    )
    parser.add_argument(
        "--checkpointing_steps",
        type=int,
...
def main(args):
    ...
    text_encoder_cls = import_model_class_from_model_name_or_path(args.pretrained_model, args.revision)
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(project_name="dreambooth-hackaton", config=wandb_cfg(args))
            ...
            if accelerator.is_main_process:
                if global_step % args.save_images_steps  == 0:
                    ...
                    pipeline = StableDiffusionPipeline(
                        ...
                        tokenizer=tokenizer,
                        scheduler=scheduler,
                        safety_checker=StableDiffusionSafetyChecker.from_pretrained(args.pretrained_model),
                    ...
                    for _ in range(num_cols):
                        images = pipeline(f"{args.instance_prompt} in the White House", guidance_scale=9).images
                        all_images.extend(images)

                    img_grid = image_grid(all_images, 1, num_cols)
...
#  --instance_prompt="a photo of sks lama" \
#  --resolution=512 \
#  --center_crop \
#  --train_batch_size=1 \
